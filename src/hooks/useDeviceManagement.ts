...
export function useDeviceManagement(): UseDeviceManagementReturn {
  ...
  const handleAddDevice = useCallback(
    async (
      ...
      try {
        ...
        console.error(
          "Failed to initialize seasonal presets for device:",
          error
        );
  ...
  const handleUpdateDevice = useCallback(
    (deviceId: number, updates: Partial<Device>) => {
      setDevices((prevDevices) => {
    ...
    async (
      ...
      if (id && devices.length > 0) {
        ...
        if (newActiveDevice && settings) {
          const deviceSeasonalPresets = await loadDeviceSeasonalPresets(
            newActiveDevice
          );
          updateSettings({
            ...settings,
