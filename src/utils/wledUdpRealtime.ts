...
function audioToLEDColors(
  ...
  if (effectType === "spectrum") {
    ...
    let r = 0,
      ...
      b = 0;
    if (
      audioFeatures.bass > audioFeatures.mid &&
      audioFeatures.bass > audioFeatures.treble
    ) {
    ...
    for (let i = 0; i < numLeds; i++) {
      ...
      colors.push({
        ...
        b: Math.floor(
          Math.min(1, audioFeatures.treble * 1.2) * wave * 255 * 0.3
        ),
      });
    }
  }
  ...
  if (previousLedColors.length === colors.length) {
    for (let i = 0; i < colors.length; i++) {
      ...
      colors[i].r = Math.floor(
        colors[i].r * 0.6 + previousLedColors[i].r * 0.4
      );
      colors[i].g = Math.floor(
        colors[i].g * 0.6 + previousLedColors[i].g * 0.4
      );
      colors[i].b = Math.floor(
        colors[i].b * 0.6 + previousLedColors[i].b * 0.4
      );
    }
  }
...
export function sendRealtimeToWLED(
  ...
  try {
    ...
    if (ledColors.length !== numLeds) {
      logger.warn(
        `LED color mismatch: expected ${numLeds}, got ${ledColors.length}`
      );
      return;
    }
    ...
    socket.send(
      ...
      (err: any) => {
        if (err) {
          packetsDropped++;
          logger.error(
            `❌ UDP send failed to ${deviceIp}:${WLED_REALTIME_PORT}:`,
            err
          );
        } else {
          packetsSent++;
...
export function closeRealtimeSocket(): void {
  if (realtimeSocket) {
    try {
      ...
      logger.log(
        `🎨 UDP realtime socket closed (Sent: ${packetsSent}, Dropped: ${packetsDropped})`
      );
